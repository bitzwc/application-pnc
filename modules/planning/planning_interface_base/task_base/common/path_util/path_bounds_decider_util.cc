...
bool PathBoundsDeciderUtil::GetBoundaryFromSelfLane(
    ...
    bool result = GetBoundaryFromSelfLane(reference_line_info, init_sl_state, path_bound, false);
    return result;
}

//TODO:增加对空置停车位空间的边界扩展
bool PathBoundsDeciderUtil::GetBoundaryFromSelfLane(
    const ReferenceLineInfo& reference_line_info, const SLState& init_sl_state,
    PathBoundary* const path_bound, const bool change_lane_with_parking_space) {
  // Sanity checks.
  AINFO << "change_lane_with_parking_space: " << change_lane_with_parking_space;
  // AINFO << "parking space info: " << reference_line_info.;
