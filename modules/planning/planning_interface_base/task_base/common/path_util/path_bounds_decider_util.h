...
class PathBoundsDeciderUtil {
 public:
  ...
  static bool GetBoundaryFromStaticObstaclesWithNudge(
      const ReferenceLineInfo& reference_line_info,
      const SLState& init_sl_state, PathBoundary* const path_boundaries,
      std::string* const blocking_obstacle_id, double* const narrowest_width,
  ...
  //2024.6.10 add，增加对空置停车位空间的边界扩展
  static bool GetBoundaryFromSelfLane(
      const ReferenceLineInfo& reference_line_info,
      const SLState& init_sl_state, PathBoundary* const path_bound, 
      const bool change_lane_with_parking_space);
  ...
  static void GetSLPolygons(const IndexedObstacles& obstacles,
                            ...
                            const SLState& init_sl_state);
  static void GetSLPolygonsWithNudgeDecision(
      const ReferenceLineInfo& reference_line_info,
