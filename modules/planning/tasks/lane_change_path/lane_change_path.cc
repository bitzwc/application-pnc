...
apollo::common::Status LaneChangePath::Process(
  ...
  if (status != ChangeLaneStatus::IN_CHANGE_LANE) {
    AINFO << injector_->planning_context()
                  ->mutable_planning_status()
                  ->mutable_change_lane()
                  ->DebugString();
...
bool LaneChangePath::DecidePathBounds(std::vector<PathBoundary>* boundary) {
  ...
  AINFO << "config_.change_lane_with_parking_space(): " << config_.change_lane_with_parking_space();
  if(config_.change_lane_with_parking_space()){
    //adc_postion 为车辆当前位置，parking_spot_range_to_start 为搜索半径，找到的停车位信息写入parking_spaces
    //const apollo::common::PointENU& end_postion
    //double parking_spot_range_to_start
    const auto& vehicle_state = frame_->vehicle_state();
    common::PointENU adc_postion;
    adc_postion.set_x(vehicle_state.x());
    adc_postion.set_y(vehicle_state.y());
    const double parking_spot_range_to_start = 50.0;
    if (hdmap_->GetParkingSpaces(adc_postion, parking_spot_range_to_start, &parking_spaces) == 0
        && parking_spaces.size() > 0) {
        for (auto parking_space : parking_spaces) {
            auto target_parking_spot_id = parking_space->parking_space().id().id();
            AINFO << "search for parking space: " << target_parking_spot_id;
        }
    }
  }
