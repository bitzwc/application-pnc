...
bool OpenSpacePreStopDecider::CheckParkingSpotPreStop(
    ...
    for (const auto& parking_overlap : parking_space_overlaps) {
        ...
        if (parking_overlap.object_id == target_parking_spot_id) {
            ...
            double center_l;
            // 在路径上获取离停车中心点最近的SL坐标
            common::SLPoint pre_stop_point_sl;
            nearby_path.GetNearestPoint(center_point, &target_area_center_s, &center_l);
            pre_stop_point_sl.set_s(target_area_center_s);
            pre_stop_point_sl.set_l(center_l);
            Vec2d pre_stop_point_xy;
            reference_line_info->reference_line().SLToXY(pre_stop_point_sl, &pre_stop_point_xy);
            target_area_found = true;
            AINFO << "pre stop point, x:" << pre_stop_point_xy.x() << ",y:" << std::fixed << std::setprecision(2)
                  << pre_stop_point_xy.y() << ",s:" << target_area_center_s << ",l:" << center_l;
        }
    }
