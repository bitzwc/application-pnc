...
void Crosswalk::MakeDecisions(Frame* const frame,
  ...
  for (auto crosswalk_overlap : crosswalk_overlaps_) {
    ...
    if (adc_front_edge_s - crosswalk_overlap->end_s >
      ...
      ALOG << "SKIP: crosswalk_id[" << crosswalk_id
    ...
    if (finished_crosswalks.end() != std::find(finished_crosswalks.begin(),
                                               ...
                                               crosswalk_id)) {
      ALOG << "SKIP: crosswalk_id[" << crosswalk_id << "] crosswalk_end_s["
    ...
    for (const auto* obstacle : path_decision->obstacles().Items()) {
      ...
      if (stop && !is_on_lane &&
        ...
        if (obstacle_speed <= kMaxStopSpeed) {
          if (crosswalk_stop_timer[crosswalk_id].count(obstacle_id) < 1) {
            // add timestamp
            ALOG << "add timestamp: obstacle_id[" << obstacle_id
            ...
            ALOG << "stop_time: obstacle_id[" << obstacle_id << "] stop_time["
                   << stop_time << "]";
      ...
      if (stop) {
        ...
        ALOG << "wait for: obstacle_id[" << obstacle_id << "] type["
               << obstacle_type_name << "] crosswalk_id[" << crosswalk_id
               << "]";
      } else {
        ALOG << "skip: obstacle_id[" << obstacle_id << "] type["
               << obstacle_type_name << "] crosswalk_id[" << crosswalk_id
               << "]";
      }
    }
    ...
    if (!pedestrians.empty()) {
      ...
      ALOG << "crosswalk_id[" << crosswalk_id << "] STOP";
    }
  }
  ...
  for (auto crosswalk_to_stop : crosswalks_to_stop) {
    ...
    ALOG << "BuildStopDecision: crosswalk[" << crosswalk_overlap->object_id
  ...
  if (firsts_crosswalk_to_stop) {
    ...
    for (const auto& timer : crosswalk_stop_timer[crosswalk]) {
      ...
      ALOG << "UPDATE stop_time: id[" << crosswalk << "] obstacle_id["
    ...
    for (auto crosswalk_overlap : crosswalk_overlaps_) {
      if (crosswalk_overlap->start_s < firsts_crosswalk_to_stop->start_s) {
        ...
        ALOG << "UPDATE finished_crosswalk: " << crosswalk_overlap->object_id;
      }
    }
  }

  ALOG << "crosswalk_status: " << mutable_crosswalk_status->DebugString();
}
...
bool Crosswalk::CheckStopForObstacle(
  ...
  if (obstacle_type != PerceptionObstacle::PEDESTRIAN &&
    ...
    ALOG << "obstacle_id[" << obstacle_id << "] type[" << obstacle_type_name
           << "]. skip";
    return false;
  }
  ...
  if (obstacle_l_distance >= config_.stop_loose_l_distance()) {
    ...
    if (is_path_cross) {
      stop = true;
      ALOG << "need_stop(>=l2): obstacle_id[" << obstacle_id << "] type["
             << obstacle_type_name << "] crosswalk_id[" << crosswalk_id << "]";
    }
    ...
    if (is_on_road) {
      ...
      if (obstacle_sl_point.s() > adc_end_edge_s) {
        stop = true;
        ALOG << "need_stop(<=l1): obstacle_id[" << obstacle_id << "] type["
    ...
    ALOG << "need_stop(between l1 & l2): obstacle_id[" << obstacle_id
