...
void PrintCurves::AddPoint(std::string key, const apollo::common::math::Vec2d& point) {
    if (!FLAGS_enable_print_curve) {
        return;
    }
    if (curve_map_.count(key) == 0) {
...
void PrintCurves::AddPoint(std::string key, const std::vector<common::math::Vec2d>& points) {
    if (!FLAGS_enable_print_curve) {
        return;
    }
...
void apollo::planning::PrintBox::AddAdcBox(double x, double y, double heading, bool is_rear_axle_point) {
    const auto& vehicle_param = apollo::common::VehicleConfigHelper::GetConfig().vehicle_param();
    if (is_rear_axle_point) {
        // rear center
        double rear_axle_to_center = vehicle_param.front_edge_to_center() - vehicle_param.length() / 2.0;
        x += rear_axle_to_center * cos(heading);
        y += rear_axle_to_center * sin(heading);
    }
    box_points.push_back({x, y, heading, vehicle_param.length(), vehicle_param.width()});
}
