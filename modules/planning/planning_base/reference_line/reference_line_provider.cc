...
ReferenceLineProvider::ReferenceLineProvider(
    ...
    if (nullptr == reference_line_config || reference_line_config->pnc_map_class().empty()) {
        const auto &pnc_map
                = apollo::cyber::plugin_manager::PluginManager::Instance()->CreateInstance<planning::PncMapBase>(
...
bool ReferenceLineProvider::UpdatePlanningCommand(const planning::PlanningCommand &command) {
    ...
    if (nullptr == current_pnc_map_) {
        AERROR << "Cannot find pnc map to process input command!" << command.DebugString();
        return false;
    }
    ...
    if (current_pnc_map_->IsNewPlanningCommand(command)) {
        ...
        if (!current_pnc_map_->UpdatePlanningCommand(command)) {
            AERROR << "Failed to update routing in pnc map: " << command.DebugString();
            return false;
        }
    }
...
bool ReferenceLineProvider::GetReferenceLinesFromRelativeMap(
    ...
    for (const auto &path_pair : relative_map_->navigation_path()) {
        ...
        ADEBUG << "lane_id = " << lane_id << " priority = " << priority << " adc_lane_id = " << adc_lane_id
    ...
    if (!high_priority_lane_pairs.empty()) {
        std::sort(
                high_priority_lane_pairs.begin(),
    ...
    if (is_lane_change_needed) {
        ...
        if (left_neighbor_lane_ids.end()
            != std::find(left_neighbor_lane_ids.begin(), left_neighbor_lane_ids.end(), target_lane_pair.first)) {
            // take the id of the first adjacent lane on the left of adc as
            // the nearest_neighbor_lane_id
            lane_change_type = routing::LEFT;
            nearest_neighbor_lane_id = adc_lane_way_point.lane->lane().left_neighbor_forward_lane_id(0).id();
        } else if (
                right_neighbor_lane_ids.end()
                != std::find(right_neighbor_lane_ids.begin(), right_neighbor_lane_ids.end(), target_lane_pair.first)) {
            ...
            nearest_neighbor_lane_id = adc_lane_way_point.lane->lane().right_neighbor_forward_lane_id(0).id();
        }
    }
...
bool ReferenceLineProvider::GetNearestWayPointFromNavigationPath(
    ...
    const int status = hdmap->GetLanesWithHeading(point, kMaxDistance, state.heading(), M_PI / 2.0, &lanes);
    if (status < 0) {
    ...
    std::copy_if(lanes.begin(), lanes.end(), std::back_inserter(valid_lanes), [&](hdmap::LaneInfoConstPtr ptr) {
...
bool ReferenceLineProvider::CreateReferenceLine(
    ...
    {
        // lock是一个锁管理器，vehicle_state_mutex_是一个锁实例
        std::lock_guard<std::mutex> lock(vehicle_state_mutex_);
        ...
        for (auto iter = segments->begin(); iter != segments->end();) {
            ...
            if (!SmoothRouteSegment(*iter, &reference_lines->back())) {
                ...
                if (!reference_lines->back().XYToSL(
                            vehicle_state.heading(), common::math::Vec2d(vehicle_state.x(), vehicle_state.y()), &sl)) {
...
bool ReferenceLineProvider::ExtendReferenceLine(
    ...
    if (!prev_segment->GetProjection(vec2d, state.heading(), &sl_point, &waypoint)) {
    ...
    if (!current_pnc_map_->ExtendSegments(*prev_segment, future_start_s, future_end_s, &shifted_segments)) {
        lock.unlock();
...
bool ReferenceLineProvider::Shrink(const common::SLPoint &sl, ReferenceLine *reference_line, RouteSegments *segments) {
...
AnchorPoint ReferenceLineProvider::GetAnchorPoint(const ReferenceLine &reference_line, double s) const {
    ...
    if (safe_lane_width < kEpislon) {
        ADEBUG << "lane width [" << left_width + right_width << "] " << "is smaller than adc width [" << adc_width
               << "]";
...
bool ReferenceLineProvider::SmoothRouteSegment(const RouteSegments &segments, ReferenceLine *reference_line) {
